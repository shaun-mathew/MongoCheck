...
comment -> ("$comment" | quote "$comment" quote | quote2 "$comment" quote2)  _ ":" _ string_a

comparison_op_eq -> ("$eq" | quote "$eq" quote | quote2 "$eq" quote2) _ ":" _ value
comparison_op_gt  -> ("$gt" | quote "$gt" quote | quote2 "$gt" quote2) _ ":" _ (num_a | string_a)
comparison_op_gte -> ("$gte" | quote "$gte" quote | quote2 "$gte" quote2) _ ":" _ (num_a | string_a)
comparison_op_in -> ("$in" | quote "$in" quote | quote2 "$in" quote2) _ ":" _ array
comparison_op_lt -> ("$lt" | quote "$lt" quote | quote2 "$lt" quote2) _ ":" _ (num_a | string_a)
comparison_op_lte -> ("$lte" | quote "$lte" quote | quote2 "$lte" quote2) _ ":" _ (num_a | string_a)
...
element_op_exists -> ("$exists" | quote "$exists" quote | quote2 "$exists" quote2) _ ":" _ boolean_a
element_op_type -> ("$type" | quote "$type" quote | quote2 "$type" quote2) _ ":" _ (num_a | array_of_string | array_of_num | string_a)
...
array_op_size -> ("$size" | quote "$size" quote | quote2 "$size" quote2) _ ":" _ num_a
...
code -> "function" _ "(" _ (key_a ( "," _ key_a):*):? _ ")" _ "{" .:* "}" | string_a

field -> id_a | key_a

value ->
	string_a | num_a | object | array | boolean_a | "null" | object_lit

array -> "[" _ (value ( _ "," _ value):*):? _ "]"

array_of_primitives -> array_of_string | array_of_bool | array_of_num

array_of_string -> "[" _ (string_a ( _ "," _ string_a):*):? _ "]"
array_of_num -> "[" _ (num_a ("," _ num_a):*):? _ "]"
array_of_bool -> "[" _ (boolean_a ( _ "," _ boolean_a):*):?  _ "]"
...
text_params -> "$search" _ ":" _ string_a _ "," _ "$language" _ ":" _ string_a _ "," _
    "$caseSensitive" _ ":" _ boolean_a _ "," _ "$diacriticSensitive" _ ":" _ boolean_a
...
meta_start -> "4SKfnQV8CbM"
meta_end -> "g3d7NksgPKQ"


id_a -> id | meta_start " " id " " meta_end
key_a -> key | meta_start " " key " " meta_end
string_a -> string | meta_start " " string " " meta_end
num_a -> num | meta_start " " num " " meta_end
boolean_a -> boolean | meta_start " " boolean " " meta_end